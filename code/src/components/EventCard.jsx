  ...
  return (
    <>
      <div
        ...
        style={{ marginBottom: 14, overflow: 'hidden', border: '1px solid #EBEBEB' }}
      >
        {/* Hero Banner */}
        <div
          className={heroClass}
        ...
        >
          ...
          >
            <div style={{ fontSize: 9, fontWeight: 700, color: '#FF5A5F', letterSpacing: '0.08em' }}>
              {monthStr}
            </div>
        ...
        <div style={{ padding: '14px 14px 14px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
            <span style={{ fontSize: 12, fontWeight: 700, color: '#767676', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
              {event.society_name}
            </span>
          ...
          {event.tagline && (
            <p style={{ fontSize: 13, color: '#767676', margin: '0 0 10px' }}>
              {event.tagline}
            </p>
          )}

          {/* Venue & time */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, fontSize: 12, color: '#767676', marginBottom: 12, flexWrap: 'wrap' }}>
            {event.venue && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                <MapPin size={13} style={{ color: '#B0B0B0' }} />
          ...
          {/* Description */}
          <p style={{ fontSize: 13, color: '#484848', lineHeight: 1.55, margin: '0 0 14px' }}>
            {event.description}
          </p>
          ...
          <div style={{ marginBottom: 14 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, marginBottom: 6, fontWeight: 600 }}>
              <span style={{ display: 'flex', alignItems: 'center', gap: 4, color: '#767676' }}>
                <Users size={11} style={{ color: '#B0B0B0' }} />
                {registeredCount} / {maxCapacity} registered
              </span>
              <span style={{ color: isAlmostFull ? '#FC642D' : '#B0B0B0', fontWeight: 700 }}>
            ...
            >
              <div
                style={{
                  height: '100%',
                  borderRadius: '9999px',
                  backgroundColor: isAlmostFull ? '#FC642D' : '#FF5A5F',
          ...
          {event.itinerary && event.itinerary.length > 0 && (
            <div style={{ marginBottom: 14 }}>
              ...
              >
                <span style={{ fontSize: 12, fontWeight: 600, color: '#484848' }}>
                  Itinerary & Timeline ({event.itinerary.length} items)
                </span>
                <motion.div animate={{ rotate: isExpanded ? 180 : 0 }} transition={{ duration: 0.2 }}>
                  <ChevronDown size={14} style={{ color: '#B0B0B0' }} />
                </motion.div>
              </button>
                    ...
                    >
                      {event.itinerary.map((item, idx) => (
                        <div key={idx} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, fontSize: 12 }}>
                          ...
                          <span style={{ color: '#484848', lineHeight: 1.4 }}>{item.activity}</span>
                        </div>
                      ))}
                    </div>
                  </motion.div>
